...
class TablePart extends StatelessWidget {
  ...
  TablePart(this.tableData, this.style);
  ...
  Widget build(BuildContext context) {
    ...
    for (var r = 0; r < tableData["rows_count"]; r++) {
      for (var c = 0; c < tableData["cols_count"]; c++) {
        ...
        if (r == 0) {
          cells.add(Text(cellText, style: style.copyWith(fontWeight: FontWeight.bold), textAlign: columnTextAlign));
        } else {
          cells.add(Text(cellText, style: style, textAlign: columnTextAlign,));
        }
      }
    ...
    if (tableData["caption"].length != 0) {
      table.add(Text(tableData["caption"],style: style,));
    }
